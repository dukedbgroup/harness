...
if [ "$IS_MASTER" == "true" ]; then
  ...
  # HBase
  head -n 1 /root/SLAVE_NAMES.txt > $HBASE_HOME/conf/regionservers
  $HBASE_HOME/bin/hbase-daemon.sh --config $HBASE_HOME/conf start zookeeper
  $HBASE_HOME/bin/hbase-daemon.sh --config $HBASE_HOME/conf start master
  $HBASE_HOME/bin/hbase-daemons.sh --hosts $HBASE_HOME/conf/regionservers start regionserver
else
  # SLAVE
  # Prep Ganglia
  sed -i --follow-symlinks -e "s|\( *mcast_join *=.*\)|#\1|" \
