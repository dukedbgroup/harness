...
EC2_INIT_SCRIPT="/home/ubuntu/ec2_hadoop_slave_init.sh"
...
for slave_private_ip in `cat "$HOSTLIST"`; do
 {
  ...
  (
   ...
   for (( attempt_num=0; attempt_num < $NUM_SSH_ATTEMPTS; attempt_num++ )) ; do
     REPLY=`ssh $SSH_OPTS "ubuntu@${slave_private_ip}" 'echo "hello"'`
     if [ ! -z $REPLY ]; then
        # ssh worked 
   ...
   if [ $MANUAL_START_REQUIRED = 0 ]; then
       ...
       scp $SSH_OPTS $EC2_INIT_SCRIPT "ubuntu@${slave_private_ip}:${EC2_INIT_SCRIPT}"
       ssh $SSH_OPTS "ubuntu@${slave_private_ip}" "sudo ${EC2_INIT_SCRIPT}"
   else 
...
wait
  ...
  echo "      master node on EC2 and run /home/ubuntu/start_hadoop_on_slaves.sh"
  echo "*********************************************************************"
  echo ""
